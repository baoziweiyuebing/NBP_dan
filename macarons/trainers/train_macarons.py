...
def setup_camera(params,
                 mesh, mesh_for_check, proxy_scene,
                 settings,
                 occupied_pose_data,
                 device, is_master,
    ...
    # Select a random camera pose as starting pose inside the 3d mesh
    next_idx_flag = True
    while next_idx_flag:
        pose_outside_mesh = True
        while pose_outside_mesh:
        ...
        # next_idx = valid_neighbors[np.random.randint(low=0, high=len(valid_neighbors))] Antoine's code
        for neigh in valid_neighbors:
            if not line_segment_mesh_intersection(camera.pose_space[str(neigh.cpu().numpy().tolist())][:3], 
                                                camera.pose_space[str(start_cam_idx.cpu().numpy().tolist())][:3], 
                                                mesh_for_check):
                next_idx = neigh
                next_idx_flag = False
                break
...
def loop(params, batch, mesh, mesh_for_check,
    ...
    for neighbor_i in range(len(valid_neighbors)):
        ...
        if not line_segment_mesh_intersection(X_neighbor.view(-1), camera.X_cam.view(-1), mesh_for_check):
            # ...We predict its coverage gain...
            with torch.no_grad():
                _, _, _, visibility_gains, coverage_gain = predict_coverage_gain_for_single_camera(
    ...
    # print("depth shape:", depth.shape[0])
    # torch.tensor()
    ...
    if params.online_learning and (not freeze):
        ...
        for i in range(depth.shape[0]):
            ...
            # ...We predict its coverage gain...
            _, _, _, visibility_gains, predicted_coverage_gain = predict_coverage_gain_for_single_camera(
                params=params, macarons=macarons.scone,
                proxy_scene=proxy_scene, surface_scene=surface_scene,
                X_world=X_world, proxy_view_harmonics=view_harmonics, occ_probs=occ_probs,
                camera=camera, X_cam_world=X_cam, fov_camera=fov_camera)
            if len(predicted_coverage_gain) > 0:
                predicted_coverage_gains[i:i+1] += predicted_coverage_gain.to(device)
...
def recompute_mapping(params, macarons,
    ...
    with torch.no_grad():
        ...
        for frame_nb in range(params.n_alpha+1, camera.n_frames_captured-1): #更改是为了cvpr
            # Load input RGB image and camera pose
            if not params.use_perfect_depth:
                all_images, all_mask, all_R, all_T, all_zfar = load_images_for_depth_model(camera=camera,
                                                                                       n_frames=1,
            ...
            for i in range(depth.shape[0]):
                ...
                if return_colors:
                    ...
                    part_pc_idx = torch.full((part_pc.shape[0], 1), frame_nb, device=device)
    ...
    if compute_coarse_mapping:
        ...
        return full_pc, full_pc_colors, full_pc_idx
    else:
        return full_pc
...
def train(params,
    ...
    for batch, scene_dict in enumerate(dataloader):
        ...
        for i_scene in range(batch_size):
            ...
            # load mesh via trimesh for inside checking
            ...
            camera = setup_camera(params, mesh, mesh_for_check, proxy_scene, settings, occupied_pose_data,
            ...
            for pose_i in range(params.n_poses_in_trajectory):
                ...
                # ----------Main loop-----------------------------------------------------------------------------------
                loss, depth_loss, occ_loss, cov_loss = loop(params, pose_i, mesh, mesh_for_check, camera,
                                                            surface_scene, proxy_scene,
...
def run_training(ddp_rank=None, params=None):
    ...
    if params.data_path is None:
        dataset_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), "../../data/doom_1")
    else:
        dataset_path = params.data_path

    # Create dataloader

    doom_scenes_name = get_subfolder_names(dataset_path)
    params.train_scenes = doom_scenes_name[:4]
    params.val_scenes = None
    params.test_scenes = None
    ...
    for t_e in range(params.epochs):
        ...
        if is_master:
            ...
            model_save_path = "sy_unvalidated_" + params.macarons_model_name + ".pth"
            ...
            if current_loss < best_train_loss:
                best_model_save_path = "sy_best_unval_" + params.macarons_model_name + ".pth"
            ...
            if t % params.save_model_every_n_epoch == 0:
                epoch_t_model_name = "sy_epoch_" + str(t) + "_" + params.macarons_model_name + ".pth"
    ...
    t666 = time.time()
    print("training time for one epoch")
    print(t666 - t222)